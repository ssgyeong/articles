...
//        if(ObjectUtils.isEmpty(findComment.get("dto"))) throw new BadRequestException("댓글 조회에 실패했습니다.");
    ...
    public ResponseEntity<?> commentUpdate(@PathVariable("commentId")Long commentId,
                                           @RequestBody CommentDTO dto) {
    ...
    public ResponseEntity<?> commentDelete(@PathVariable("commentId")Long commentId) {
        //삭제할 댓글 존재 확인
        CommentDTO result = extracted(commentId, "댓글 삭제가 실패하였습니다.");
    ...
    private CommentDTO extracted(Long commentId, String message) {
        //댓글 존재 확인
        Map<String, Object> findComment = commentService.findByIdComment(commentId);
        if(ObjectUtils.isEmpty(findComment.get("dto"))) throw new BadRequestException(message);
